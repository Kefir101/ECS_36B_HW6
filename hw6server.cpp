...
bool checkWord(const char *arg_guess)
{
...
bool isWord(const char *arg_word)
{
  int i = -1;
...
bool hasWord(const char *arg_word)
{
...
Json::Value
Myhw6Server::set_name(const std::string &action, const Json::Value &student_list,
                      const std::string &team_name)
{
  ...
  try
  {
    ...
    {
      error_code = rc;
      throw ecs36b_Exception{("myFile2JSON error " + std::string{buf_fname1})};
    }

    if (!lv_teams.isNull() && !lv_teams.isObject())
    {
      error_code = -1;
      throw ecs36b_Exception{("teams JSON format incorrect " + std::string{buf_fname1})};
    }
    ...
    {
      error_code = rc;
      throw ecs36b_Exception{("myFile2JSON error " + std::string{buf_fname2})};
    }

    if (lv_students.isNull() || !lv_students.isObject())
    {
      error_code = -2;
      throw ecs36b_Exception{("students JSON format incorrect " + std::string{buf_fname2})};
    }
    ...
    if (student_list.isNull() || !student_list.isArray() || student_list.size() == 0)
    {
      error_code = -4;
    ...
    {
      error_code = -5;
      throw ecs36b_Exception{("team name already taken: " + team_name)};
    }

    // set up a new team, check if the team members have been registered with another team
    Json::Value lv_jv;
    for (i = 0; i < student_list.size(); i++)
    {
      ...
      {
        error_code = -6;
        throw ecs36b_Exception{("student name is not a string, index " + std::to_string(i))};
      }
      else
      {
        //commented out because doing x = jsonValues[key] usually results in a new k:v pair with value as null, so the code throws error
        lv_jv = lv_students[student_list[i].asString()];
        if (lv_jv.isNull() || !lv_jv.isObject())
        {
          error_code = -7;
          throw ecs36b_Exception{("student format incorrect. index = " + std::to_string(i))};
        }
        if ((lv_jv["team name"].isNull()) ||
            (lv_jv["team name"].isString() == false) ||
            (lv_jv["team name"]).asString() != "unassigned")
        {
          error_code = -8;
          throw ecs36b_Exception{("team_name not unassigned. index = " + std::to_string(i))};
        }
      }
    }
    for (i = 0; i < student_list.size(); i++)
    {
      lv_students[student_list[i].asString()]["team name"] = team_name;
      lv_students[student_list[i].asString()]["score"] = 0;
    }
    ...
    {
      error_code = rc;
      throw ecs36b_Exception{("myJSON2File error " + std::string{buf_fname1})};
    }
    ...
    {
      error_code = rc;
      throw ecs36b_Exception{("myJSON2File error " + std::string{buf_fname2})};
    }
  }
...
Json::Value
Myhw6Server::submit(const std::string &action, const std::string &game_id,
                    const std::string &team_name)
{
  Json::Value lv_log;
  lv_log["arg action"] = action;
  ...
  try
  {
    ...
    if ((result["remaining_score"].isNull()) ||
    ...
    if ((result["score_distributed"].isNull()) ||
        (result["score_distributed"].isBool() == false) ||
        (result["score_distributed"].asBool()))
    {
      error_code = -5;
        ...
        (lv_team_list.isArray()))
    {
      ...
      {
        error_code = rc;
        throw ecs36b_Exception{("myJSON2File error " + std::string{buf_fname})};
      }
...
Json::Value
Myhw6Server::obtain(const std::string &action, const std::string &class_id,
...
Json::Value
Myhw6Server::guess(const std::string &action, const std::string &class_id,
